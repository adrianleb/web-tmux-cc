        ...
        reseedPane(id) {
          ...
          reseedTimer = window.setTimeout(() => {
            ...
            for (const paneId of ids) {
              panes.get(paneId).captureGridKey = panes.get(paneId).gridKey
    ...
    function reportGrid(store, body) {
      ...
      const rows = Math.max(6, Math.floor(rect.height / cell.h) - 1)
    ...
    function mountPane(host, pane, store, layout, place) {
      ...
      if (!rec) {
        ...
        host.append(wrap)
        rec = {
          wrap, title, label, termHost, touchLayer, term: null, seeded: false, fitKey: '', layout: null,
          onTap: () => {
            store.send({ type: 'select', pane: pane.id })
